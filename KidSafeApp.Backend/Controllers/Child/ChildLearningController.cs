...
namespace KidSafeApp.Backend.Controllers.Child;
...
public sealed class ChildLearningController : ControllerBase
{
    ...
    [HttpGet("courses")]
    public async Task<ActionResult<List<ChildCourseDto>>> GetPublishedCourses([FromQuery] int? childId = null, CancellationToken cancellationToken = default)
    {
        var actingChildId = childId.GetValueOrDefault();
        if (actingChildId <= 0)
        {
            actingChildId = await GetActingChildIdAsync(cancellationToken);
        }

        var assignedCourseIds = await _context.CourseAssignments
            .AsNoTracking()
            .Where(a => a.ChildId == actingChildId && a.IsActive)
            .Select(a => a.CourseId)
            .ToListAsync(cancellationToken);

        var courses = await _context.Courses
            .AsNoTracking()
            .Where(c => c.IsPublished && assignedCourseIds.Contains(c.Id))
        ...
        var progressByCourse = await _context.LessonProgresses
            .AsNoTracking()
            .Where(lp => lp.ChildId == actingChildId && courseIds.Contains(lp.CourseLesson.CourseId))
