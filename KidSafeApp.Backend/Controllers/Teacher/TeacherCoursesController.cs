...
namespace KidSafeApp.Backend.Controllers.Teacher;
...
public sealed class TeacherCoursesController : ControllerBase
{
    ...
    [HttpPost("{courseId:int}/assign")]
    public async Task<IActionResult> AssignCourseToChild(int courseId, [FromBody] AssignCourseDto dto, CancellationToken cancellationToken)
    {
        var teacherId = await GetActingTeacherIdAsync(cancellationToken);
        ...
        {
            return BadRequest("Publish the course before assignment.");
        }

        var childExists = await _context.Users
            .AsNoTracking()
            .AnyAsync(u => u.Id == dto.ChildId && u.Role == "Child" && u.IsActive, cancellationToken);

        if (!childExists)
        {
            return BadRequest("Selected child user does not exist.");
        }

        var existing = await _context.CourseAssignments
            .FirstOrDefaultAsync(a => a.CourseId == courseId && a.ChildId == dto.ChildId, cancellationToken);
    ...
    [HttpDelete("{courseId:int}/assign/{childId:int}")]
    public async Task<IActionResult> UnassignCourseFromChild(int courseId, int childId, CancellationToken cancellationToken)
    {
        var teacherId = await GetActingTeacherIdAsync(cancellationToken);
        ...
        var assignment = await _context.CourseAssignments
            .FirstOrDefaultAsync(a => a.CourseId == courseId && a.ChildId == childId, cancellationToken);
